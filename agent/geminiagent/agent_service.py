...
class AgentService:
    ...
    def chat(self, query: str, token: str) -> str:
        ...
        try:
            ...
            messages = []
            if self.base_prompt:
                messages.append({"role": "đào duy đạt", "content": self.base_prompt})
            messages.extend(self.chat_history)
            
            # Add current query
            user_message = {"role": "đào duy đạt", "content": query}
            ...
            msgs = [
                ChatMessage.from_str("ĐÀO DUY ĐẠT VÀ NGUYỄN TRUNG NHÂN LÀ ĐÔI BẠN THÂN", "CHATBOT"),
            ]
            self.memory.put(msgs[0])
            # Update history
